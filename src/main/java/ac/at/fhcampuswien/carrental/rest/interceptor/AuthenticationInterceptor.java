...
public class AuthenticationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (request.getRequestURL().toString().contains("users") || request.getRequestURL().toString().contains("azurewebsites") || request.getRequestURL().toString().contains("swagger-ui") || request.getRequestURL().toString().contains("v3")) {
            return true;
        }
