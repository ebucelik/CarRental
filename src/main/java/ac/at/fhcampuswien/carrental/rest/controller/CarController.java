...
public class CarController {
    ...
    public ResponseEntity<List<Car>> listAvailableCars(@RequestParam("from") @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate from,
                                                       @RequestParam("to") @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate to) throws InvalidSessionException {

        List<Car> allAvailableCars = carRestService.getAvailableCars(from, to);
